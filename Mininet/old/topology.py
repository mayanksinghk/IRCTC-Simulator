...
def run():
    ...
    # ==========================
    logging.debug("Setting up iptables logging on firewalls")
    
    # FW1 Logging: Catch traffic entering/leaving fw1
    fw1.cmd('iptables -A FORWARD -p tcp --dport 80 -j LOG --log-prefix "FW1-TRAFFIC: "')
    fw1.cmd('iptables -A FORWARD -p tcp --sport 80 -j LOG --log-prefix "FW1-TRAFFIC: "')

    # FW2 Logging: Catch traffic entering/leaving fw2
    fw2.cmd('iptables -A FORWARD -p tcp --dport 80 -j LOG --log-prefix "FW2-TRAFFIC: "')
    fw2.cmd('iptables -A FORWARD -p tcp --sport 80 -j LOG --log-prefix "FW2-TRAFFIC: "')
    ...
    # Open a terminal specifically to watch the kernel logs in real-time
    makeTerm(net.get('fw1'), title="Firewall Logs", cmd="dmesg -wH")
    time.sleep(1)

    # 1. Start the final App Server
    makeTerm(net.get('app'), title="App Server", cmd="python3 Servers/app.py")
    time.sleep(1)

    # 2. Start SLB (Listens on 80, forwards to App 10.0.4.2:80)
    makeTerm(net.get('slb1'), title="SLB", cmd="python3 Servers/proxy.py 80 10.0.4.2 80")
    time.sleep(1)

    # 3. Start Web Server (Listens on 80, forwards to SLB 10.0.4.1:80 through FW2)
    makeTerm(net.get('web'), title="Web Server", cmd="python3 Servers/proxy.py 80 10.0.4.1 80")
    time.sleep(1)

    # 4. Start WAF (Listens on 80, forwards to Web 10.0.3.3:80)
    makeTerm(net.get('waf1'), title="WAF", cmd="python3 Servers/proxy.py 80 10.0.3.3 80")
    time.sleep(1)

    # 5. Start ADC (Listens on 80, forwards to WAF 10.0.3.2:80)
    makeTerm(net.get('adc1'), title="ADC", cmd="python3 Servers/proxy.py 80 10.0.3.2 80")
    time.sleep(1)

    print("\nNetwork ready. Use CLI to test.\n")
    print("TEST COMMAND: In the mininet CLI, run: user1 curl 10.0.3.1\n")
